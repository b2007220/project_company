...
<script>
    ...
    document.getElementById("productForm").addEventListener("submit", function(event) {
        ...
        const method = productForm.querySelector('input[name="_method"]') ? 'PUT' : 'POST';
        $.ajaxSetup({
            headers: {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (method === 'PUT') {
                    ...
                    if (row) {
                        row.innerHTML = `<td class="px-6 py-4 whitespace-no-wrap border-bottom border-gray-200 overflow-auto max-w-sm text-sm">
                    <div
                ...
                <td class="px-6 py-4 text-sm leading-5 text-gray-500 whitespace-no-wrap border-bottom border-gray-200">
                    <div
                        ...
                        ${result.product.categories.map(category => `<span> ${category.name}</span>`).join('')}
                    </div>
                ...
                <td
                    ...
                    <div
                        ...
                        ${result.discounts.count() > 0 ? result.discounts.map(discount => discount.is_predefined ? `<span>${(result.product.price - (result.product.price * discount.discount) / 100).toLocaleString('en-US')} đ</span>` : '').join('') : `<span>${result.product.price.toLocaleString('en-US')} đ</span>`}
                    </div>
            ...
            <tr class="detail-${result.product.id} collapse ">
                <td colspan="12">
                    ...
                    <form  method="POST"
                        ...
                        <div class="d-flex align-items-center px-6 gap-4 flex-column-max-lg" id="pictures-container">
                            ...
                            ${result.product.pictures.map(picture => `<div class="border rounded border-gray-300 picture-item" data-id="${picture.id}">
                    ...
                    <table class="w-100">
                        ...
                        <tbody>
                            ${result.product.discounts.map(discount => `
                                <tr>
                                    <td
                                        class="px-6 py-4 whitespace-no-wrap border-bottom border-gray-200 overflow-auto max-w-sm text-sm leading-5 text-gray-500 ">
                                        <div
                ...
                `;
                    ...
                    newRow.setAttribute('data-product-id', result.product.id);
                    newRow.setAttribute('data-bs-toggle', 'collapse');
                    newRow.setAttribute('data-bs-target', `.detail-${result.product.id}`);
                    newRow.innerHTML = `<td class="px-6 py-4 whitespace-no-wrap border-bottom border-gray-200 overflow-auto max-w-sm text-sm">
                    <div
                        class="ml-4 text-sm leading-5 text-gray-900 font-medium d-flex justify-content-center align-items-center">
                        ${result.product.name}
                    </div>
